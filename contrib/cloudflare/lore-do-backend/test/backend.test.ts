...
describe("Lore Durable Objects backend", () => {
  it("requires a fresh valid HMAC signature", async () => {
    ...
    const stale = await api("/v1/mutable/load", {
      partition: PARTITION,
      key: hash(1),
      keyType: 0,
  ...
  it("preserves ordered match specificity across immutable shards", async () => {
    ...
    expect((await api("/v1/immutable/put", { partition: PARTITION, address: first, fragment })).status).toBe(200);
    expect((await api("/v1/immutable/put", { partition: PARTITION, address: second, fragment })).status).toBe(200);
    ...
    await expect(crossPartition.json()).resolves.toMatchObject({ matchMade: 1 });
  });

  it("allows one mutable CAS winner and deletes on the null hash", async () => {
    const key = hash(20);
    const requests = [hash(21), hash(22)].map((value) => api("/v1/mutable/compare-and-swap", {
      partition: PARTITION,
      key,
      expected: ZERO_HASH,
      value,
    ...
    const bodies = await Promise.all(responses.map(async (response) => response.json() as Promise<{ swapped: boolean }>));
    expect(bodies.filter((body) => body.swapped)).toHaveLength(1);

    const loaded = await api("/v1/mutable/load", { partition: PARTITION, key, keyType: 3 });
    const loadedBody = await loaded.json() as { value: string };
    expect([hash(21), hash(22)]).toContain(loadedBody.value);

    await api("/v1/mutable/store", { partition: PARTITION, key, value: ZERO_HASH, keyType: 3 });
    await expect((await api("/v1/mutable/load", { partition: PARTITION, key, keyType: 3 })).json())
  ...
  it("stores payloads privately under the signed service API", async () => {
    ...
    expect((await rawApi("PUT", `/v1/payload/${payloadHash}`, payload)).status).toBe(200);
    expect((await rawApi("PUT", `/v1/payload/${payloadHash}`, payload)).status).toBe(200);
    expect((await rawApi("PUT", `/v1/payload/${payloadHash}`, new Uint8Array([9, 9]))).status).toBe(409);

    const loaded = await rawApi("GET", `/v1/payload/${payloadHash}`, new Uint8Array());
    expect(loaded.status).toBe(200);
    expect(new Uint8Array(await loaded.arrayBuffer())).toEqual(payload);

    expect((await rawApi("DELETE", `/v1/payload/${payloadHash}`, new Uint8Array())).status).toBe(200);
    expect((await rawApi("GET", `/v1/payload/${payloadHash}`, new Uint8Array())).status).toBe(404);
  });
  ...
  it("keeps conflicting lock batches all-or-nothing", async () => {
    ...
    expect((await api("/v1/locks/acquire", {
      owner: "alice",
    ...
    const body = await status.json() as { locks: { owner: string; resource: { hash: string } }[] };
    expect(body.locks).toHaveLength(1);
    expect(body.locks[0]).toMatchObject({ owner: "alice", resource: { hash: held.hash } });
  });
});
...
async function api(path: string, body: unknown, timestamp = Math.floor(Date.now() / 1000)): Promise<Response> {
  const encoded = JSON.stringify(body);
  return rawApi("POST", path, new TextEncoder().encode(encoded), timestamp, "application/json");
}
